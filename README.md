...
-	Worked with the business to redefine the batch job schedule using Azure Functions to run based on file trigged within defined time scope, eliminationing the reasocure allocation all the times, this make sure the lamba fired up and used resporces only when needed  and also that it runs before the business time window. For example, instead of running the batch job at 11 PM PST, we triggered the batch job at 4 PM PST, the business end time, giving the batch job about 7 additional hours to process.
...
-	Leveraged Java 8 and Java 21 Features: Refactored legacy code to utilize Java 8 parallel streams, which led to more efficient data handling and reduced processing time and Java 21s virtual threads.
-	Introduced Apache Kafka for mroe asyncronized processing for file uploads and heavy data enginering system
-	Migrated this app from Monolith to Modular Monolith application
...
- Java 21, Heap Dump analyzer, SQL query analyzer, Sonar, Nexus, Whitehat Maven, Jenkins, JUnit, Artifactory, Git
